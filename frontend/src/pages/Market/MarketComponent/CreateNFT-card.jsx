//========================================================
//=====ITEM COMPONENT FOR Create-and-Sell-NFT SECTION=====
//========================================================



import React from "react";
import PropTypes from "prop-types";

const Card = ({step, title, bodytext}) => {
  return (
    <div 
      className="col-12 col-sm-12 col-md-4 col-lg-3 d-flex justify-content-center mb-3"
    >
      <div className="shadow-lg card-layout">
        <div className="d-flex ">
            <div className="item-descript">
              <div className="d-flex">
                <h5 className="outfit hl-bold-text">{step} <br /><br />{title}</h5>
              </div>
              <div className="d-flex">
                <p className="bodytext gray mb-2"><br /></p>
              </div>
              <div className="d-flex justify-content-between">
                <p className="outfit mb-0 light-gray">{bodytext}</p>
              </div>
            
            </div>
            <div>
                <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="currentColor" class="bi bi-arrow-right" viewBox="0 0 16 16">
                <path fill-rule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8"/>
                </svg>
            </div>
        </div>
      </div>
    </div>
  );
};

Card.propTypes = {
  step: PropTypes.string.isRequired,
  title: PropTypes.string.isRequired,
  bodytext: PropTypes.string.isRequired,
};


export default Card;
