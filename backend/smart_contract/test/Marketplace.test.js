...
describe("Marketplace", function () {
  ...
  beforeEach(async function () {
    ...
    marketplace = await Marketplace.deploy(myNFT.address);
    await marketplace.deployed();

    await myNFT.mintNFT(owner.address, "ipfs://test1");
    await myNFT.approve(marketplace.address, 1);
  });

  it("Should deploy with correct NFT contract address", async function () {
    expect(await marketplace.nftContract()).to.equal(myNFT.address);
  });

  describe("listNFT", function () {
    it("Should list an NFT", async function () {
      const tx = await marketplace.listNFT(1, ethers.utils.parseEther("1"));
      expect(await myNFT.ownerOf(1)).to.equal(marketplace.address);
      ...
      expect(tx).to.emit(marketplace, "NFTListed").withArgs(1, owner.address, ethers.utils.parseEther("1"));
    });

    it("Should revert if non-owner lists", async function () {
      await expect(marketplace.connect(addr1).listNFT(1, ethers.utils.parseEther("1")))
        .to.be.revertedWith("Only the owner can list the NFT");
    });

    it("Should revert if price is zero", async function () {
      await expect(marketplace.listNFT(1, 0)).to.be.revertedWith("Price must be greater than zero");
    });
  });
  ...
  describe("buyNFT", function () {
    it("Should buy a listed NFT", async function () {
      ...
      const tx = await marketplace.connect(addr1).buyNFT(1, { value: ethers.utils.parseEther("1") });
      ...
      expect(tx).to.emit(marketplace, "NFTSold").withArgs(1, addr1.address, owner.address, ethers.utils.parseEther("1"));
    });

    it("Should refund excess payment", async function () {
      await marketplace.listNFT(1, ethers.utils.parseEther("1"));
      const initialBalance = await ethers.provider.getBalance(addr1.address);
      await marketplace.connect(addr1).buyNFT(1, { value: ethers.utils.parseEther("2") });
      const finalBalance = await ethers.provider.getBalance(addr1.address);
      expect(finalBalance).to.be.closeTo(initialBalance.sub(ethers.utils.parseEther("1")), ethers.utils.parseEther("0.1"));
    });

    it("Should revert if insufficient payment", async function () {
      await marketplace.listNFT(1, ethers.utils.parseEther("1"));
      await expect(marketplace.connect(addr1).buyNFT(1, { value: ethers.utils.parseEther("0.5") }))
  ...
  describe("cancelListing", function () {
    it("Should cancel a listing", async function () {
      ...
      expect(await marketplace.listings(1)).to.include({ active: false });
      expect(await marketplace.listingCount()).to.equal(0);
      expect(tx).to.emit(marketplace, "ListingCancelled").withArgs(1, owner.address);
    });
