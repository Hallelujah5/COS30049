...
describe("Auction", function () {
  ...
  beforeEach(async function () {
    ...
    myNFT = await MyNFT.deploy(ethers.constants.AddressZero);
    await myNFT.deployed();

    Auction = await ethers.getContractFactory("Auction");
    auction = await Auction.deploy(myNFT.address);
    await auction.deployed();

    await myNFT.mintNFT(owner.address, "ipfs://test1");
    await myNFT.approve(auction.address, 1);
  });

  it("Should deploy with correct NFT contract address", async function () {
    expect(await auction.nftContract()).to.equal(myNFT.address);
  });

  describe("startAuction", function () {
    it("Should start an auction", async function () {
      const tx = await auction.startAuction(1, ethers.utils.parseEther("0.5"));
      expect(await myNFT.ownerOf(1)).to.equal(auction.address);
      const auctionDetails = await auction.auctions(1);
      expect(auctionDetails.seller).to.equal(owner.address);
      expect(auctionDetails.highestBid).to.equal(ethers.utils.parseEther("0.5"));
      expect(auctionDetails.endBlock).to.equal((await ethers.provider.getBlockNumber()) + 2);
      expect(auctionDetails.active).to.be.true;
      expect(tx).to.emit(auction, "AuctionStarted").withArgs(1, owner.address, ethers.utils.parseEther("0.5"), anyValue);
    });

    it("Should revert if non-owner starts", async function () {
      await expect(auction.connect(addr1).startAuction(1, ethers.utils.parseEther("0.5")))
  ...
  describe("bid", function () {
    it("Should place a bid", async function () {
      ...
      const tx = await auction.connect(addr1).bid(1, { value: ethers.utils.parseEther("1") });
      const auctionDetails = await auction.auctions(1);
      ...
      expect(tx).to.emit(auction, "BidPlaced").withArgs(1, addr1.address, ethers.utils.parseEther("1"));
    });

    it("Should refund previous bidder", async function () {
      await auction.startAuction(1, ethers.utils.parseEther("0.5"));
      await auction.connect(addr1).bid(1, { value: ethers.utils.parseEther("1") });
      const initialBalance = await ethers.provider.getBalance(addr1.address);
      await auction.connect(addr2).bid(1, { value: ethers.utils.parseEther("2") });
      const finalBalance = await ethers.provider.getBalance(addr1.address);
      expect(finalBalance).to.be.closeTo(initialBalance.add(ethers.utils.parseEther("1")), ethers.utils.parseEther("0.1"));
    });

    it("Should revert if bid too low", async function () {
      await auction.startAuction(1, ethers.utils.parseEther("0.5"));
      await expect(auction.connect(addr1).bid(1, { value: ethers.utils.parseEther("0.4") }))
  ...
  describe("endAuction", function () {
    it("Should end auction with a winner", async function () {
      ...
      expect(tx).to.emit(auction, "AuctionEnded").withArgs(1, addr1.address, ethers.utils.parseEther("1"));
    });
