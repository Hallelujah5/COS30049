...
describe("Transactions", function () {
  ...
  describe("addToBlockchain", function () {
    it("Should add a transaction", async function () {
      const tx = await transactions.addToBlockchain(addr1.address, ethers.utils.parseEther("1"), "Hello", "Test", {
      ...
      expect(tx).to.emit(transactions, "Transfer").withArgs(owner.address, addr1.address, ethers.utils.parseEther("1"), "Hello", "Test", anyValue);
    });

    it("Should revert if amount mismatch", async function () {
      await expect(transactions.addToBlockchain(addr1.address, ethers.utils.parseEther("1"), "Hello", "Test", {
        value: ethers.utils.parseEther("0.5"),
      })).to.be.revertedWith("Amount mismatch");
    });

    it("Should revert if receiver is zero address", async function () {
      await expect(transactions.addToBlockchain(ethers.constants.AddressZero, ethers.utils.parseEther("1"), "Hello", "Test", {
        value: ethers.utils.parseEther("1"),
  ...
  describe("receive and fallback", function () {
    it("Should receive ETH", async function () {
      await owner.sendTransaction({ to: transactions.address, value: ethers.utils.parseEther("1") });
      expect(await transactions.getContractBalance()).to.equal(ethers.utils.parseEther("1"));
    });
  });
  ...
  describe("withdrawTo", function () {
    it("Should withdraw ETH as owner", async function () {
      ...
      await transactions.withdrawTo(addr1.address, ethers.utils.parseEther("1"));
    ...
    it("Should revert if non-owner withdraws", async function () {
      await expect(transactions.connect(addr1).withdrawTo(addr1.address, ethers.utils.parseEther("1")))
        .to.be.revertedWith("Only owner can withdraw");
    });

    it("Should revert if insufficient balance", async function () {
      await expect(transactions.withdrawTo(addr1.address, ethers.utils.parseEther("1")))
  ...
  describe("View functions", function () {
    it("Should return all transactions", async function () {
      await transactions.addToBlockchain(addr1.address, ethers.utils.parseEther("1"), "Hello", "Test", {
        value: ethers.utils.parseEther("1"),
      });
      const allTxs = await transactions.getAllTransactions();
      expect(allTxs.length).to.equal(1);
